...
public class ViewFrequencyAdjust extends View {
    ...
    private Paint paintDot;//圆点画笔
    private Paint paintTouchLine;//可调节线画笔

    private int mTopUnitHeight;// 顶部Y轴单位高度
    private int mBottomTextHeight;//X轴底部字体的高度
    private int mLeftTextWidth;//Y轴左边的文字宽度
    private int yHeight, xWidth;//x y 轴的长度
    ...
    private float mAdjustLineLength;//斜线的长度
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        paintLine.setColor(getResources().getColor(R.color.view_frequency_adjust_line));

        paintText = new Paint(Paint.ANTI_ALIAS_FLAG | Paint.DITHER_FLAG);
        paintText.setColor(getResources().getColor(R.color.view_frequency_adjust_text));
        ...
        paintDot.setColor(getResources().getColor(R.color.view_frequency_adjust_line_dot));
    ...
    private void drawUI(Canvas canvas) {
        ...
        drawViewTouch(canvas);//绘制调节线
        if (isTouchOn())
            drawViewGuide(canvas);//调节引导
    ...
    private void drawBg(Canvas canvas) {
        canvas.drawColor(getResources().getColor(R.color.view_frequency_adjust_bg));
    }
    ...
    private void drawLineXY(Canvas canvas) {
        ...
        for (int i = 0; i < xLineCount; i++) {
            ...
            if (i == 0  /*||i == xLineCount - 1*/)
                canvas.drawLine(start.x, start.y, stop.x, stop.y, paintLine);//绘制横轴线
    ...
    private void drawViewTouch(Canvas canvas){
        paintTouchLine.setStrokeWidth(ResUtils.getDimension(R.dimen.view_frequency_adjust_line_touch));
    ...
    private Point calculateStartPoint(@NonNull TouchLine touchLine) {
    ...
    private Point calculateEndPoint(@NonNull TouchLine touchLine) {
